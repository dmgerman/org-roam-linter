...
- You're writing a plan to resolve a chore, it should be simple but we
  need to be thorough and precise so we don't miss anything or waste
  time with any second round of changes.

- Create the plan in the `specs/*.md` file. Name it appropriately based
  on the `Chore`. The filename should start with the today's date: 2025-11-13-<chore-name>.md
...
- IMPORTANT: Replace every <placeholder> in the `Plan Format` with the requested
  value. Add as much detail as needed to accomplish the chore.

- Use your reasoning model: THINK HARD about the plan and the steps to
...
- forClause.org has documentation of the system
- read any necessary source code files in the root directory only if necessary
